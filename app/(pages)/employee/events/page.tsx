...
export default function EventsPage() {
  ...
  const [modalEventInfos, setModalEventInfos] = useState<EventClickArg | null>(
    null
  );
  ...
  return (
    <>
      <h1 className="font-bold text-5xl md:text-3xl mb-6">Events</h1>
      <ContentWrapper>
        <FullCalendar
          ...
          events={eventsInfos.map(
            (eventInfo) => new EventInfos(eventInfo.name, eventInfo.date)
          )}
        ...
        <Modal
          ...
          onClose={closeModal}
          placement="center"
          className="rounded-[2px] border-2 border-color"
        >
          <ModalContent>
            ...
            <ModalBody>
              <Listbox
                className="flex flex-col overflow-y-scroll h-[400px]"
                aria-label="Events display"
              >
                {eventsInfos
                  .filter(
                    (eventInfos) =>
                      eventInfos.date === modalEventInfos?.event.startStr
                  )
                  .map((event) => (
                    <ListboxItem
                      key={event.id}
